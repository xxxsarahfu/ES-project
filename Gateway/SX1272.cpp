...
//    delay(250);
#if (SX1272_duration > 1)
    printf("setpacketlength\n");
    duration(spltime);
    return state;
#endif
}
...
{	// RSSIpacket only exists in LoRa
    int8_t state = 2;
    unsigned long grssitime = millis();
...
void SX1272::CarrierSense() {
    ...
    unsigned long csrtime = millis();
    printf("carriersenseretry\n");
    duration(csrtime);
...
/*
additional variable & function
*/
void duration(unsigned long t){
	printf(" cost time = %lf seconds\n", float(millis() - t) * 1/1000); 
} 
